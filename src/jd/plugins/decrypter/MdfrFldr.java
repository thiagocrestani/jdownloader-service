...
@DecrypterPlugin(revision = "$Revision: 12626 $", interfaceVersion = 2, names = { "mediafire.com" }, urls = { "http://[\\w\\.]*?(?!download)[\\w\\.]*?mediafire\\.com/(\\?sharekey=.+|(?!download|file)[^\\?]+)" }, flags = { 0 })
public class MdfrFldr extends PluginForDecrypt {
    ...
    public ArrayList<DownloadLink> decryptIt(CryptedLink param, ProgressController progress) throws Exception {
        ...
        Thread.sleep(500);
        String reqlink = br.getRegex(Pattern.compile("LoadJS\\(\"http://.*?/js/myfiles\\.php/(.*?)\"")).getMatch(0);
