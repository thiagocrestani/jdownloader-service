...
@HostPlugin(revision = "$Revision: 11261 $", interfaceVersion = 2, names = { "supashare.net" }, urls = { "http://[\\w\\.]*?supashare\\.net/[a-z0-9]{12}" }, flags = { 0 })
public class SupaShareNet extends PluginForHost {

    public SupaShareNet(PluginWrapper wrapper) {
        super(wrapper);
        // this.enablePremium(COOKIE_HOST + "/premium.html");
    }

    // XfileSharingProBasic Version 1.6 (modified filename & filesize regexes)
    @Override
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        if (filename == null) {
            ...
            if (filename == null) {
                ...
                if (filename == null) {
                    filename = br.getRegex("<strong>Name:</strong>(.*?)<br />").getMatch(0);
                    if (filename == null) {
                        filename = br.getRegex("<h2>(.*?)\\(.*?\\)</h2>").getMatch(0);
                        if (filename == null) {
                            filename = br.getRegex("<title>Download(.*?)- Supashare.net</title>").getMatch(0);
                        }
                    }
                }
            }
