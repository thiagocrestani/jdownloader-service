...
@HostPlugin(revision = "$Revision: 12369 $", interfaceVersion = 2, names = { "filekeeping.de" }, urls = { "http://[\\w\\.]*?filekeeping\\.de/((\\?d|download\\.php\\?id)=[A-Z0-9]+|((en|ru|fr|es)/)?file/[0-9]+/)" }, flags = { 0 })
public class FileKeepingDe extends PluginForHost {

    public FileKeepingDe(PluginWrapper wrapper) {
        super(wrapper);
    }
    ...
    private static final String COOKIE_HOST = "http://filekeeping.de";
    ...
    public void handleFree(DownloadLink link) throws Exception {
        ...
        if (br.containsHTML("(captcha.php|class=textinput name=downloadpw)")) {
            ...
            for (int i = 0; i <= 3; i++) {
                ...
                if (br.containsHTML("You have got max allowed download sessions from the")) throw new PluginException(LinkStatus.ERROR_IP_BLOCKED);
