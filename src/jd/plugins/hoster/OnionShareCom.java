...
@HostPlugin(revision = "$Revision: 11535 $", interfaceVersion = 2, names = { "onionshare.com" }, urls = { "http://[\\w\\.]*?onionshare\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class OnionShareCom extends PluginForHost {
    ...
    // XfileSharingProBasic Version 1.8, modified dllink, filename/size regexes
    @Override
    ...
    private static final String COOKIE_HOST = "http://onionshare.com";
    ...
    public AvailableStatus requestFileInformation(DownloadLink link) throws IOException, PluginException {
        ...
        String filename = br.getRegex("\">Name :</span>(.*?)<span").getMatch(0);
        if (filename == null) {
            filename = new Regex(brbefore, "class=\"filename\">(.*?)</div>").getMatch(0);
        }
        String filesize = new Regex(brbefore, "\">Size :</span>(.*?)</div>").getMatch(0);
        if (filesize == null) {
            filesize = new Regex(brbefore, "onFocus=\"copy\\(this\\);\"><a href=\"http://www\\.onionshare\\.com/[a-z0-9]{12}\">.*? - (.*?)</a></textarea>").getMatch(0);
        }
    ...
    public String getDllink() {
        ...
        if (dllink == null) {
            ...
            if (dllink == null) {
                ...
                if (dllink == null) {
                    dllink = new Regex(brbefore, "\"(http://www.onionserver\\d+\\.info/files/\\d+/[a-z0-9]+/.*?)\"").getMatch(0);
                    if (dllink == null) {
                        dllink = new Regex(brbefore, "<div style=\"height: 8px;\"></div>[\n\r ]+<a href=\"(http://.*?)\"").getMatch(0);
                    }
                }
            }
        }
