...
@HostPlugin(revision = "$Revision: 12818 $", interfaceVersion = 2, names = { "mangoshare.com" }, urls = { "http://[\\w\\.]*?(mangoshare|mangobit)\\.com/[a-z0-9]{12}" }, flags = { 0 })
public class MangoShareCom extends PluginForHost {

    public MangoShareCom(PluginWrapper wrapper) {
        super(wrapper);
        // this.enablePremium(COOKIE_HOST + "/premium.html");
    }

    private static final String FATALSERVERERROR = "Error happened when generating Download Link";

    // XfileSharingProBasic Version 2.0.0.0, extended errorhandling
    @Override
    ...
    private static final String COOKIE_HOST                      = "http://mangobit.com";
    public boolean              NOPREMIUM                        = false;
    private static final String EMERGENCYPICTUREDOWNLOADLINKTEXT = "emergencypicturelink";

    public void correctDownloadLink(DownloadLink link) throws IOException {
        String theLink = link.getDownloadURL();
        if (theLink.contains("mangoshare.com")) {
            br.getPage(theLink);
            String newLink = br.getRegex("\\'(http://mangobit\\.com/[a-z0-9]{12}\\.html)\\'").getMatch(0);
            if (newLink != null) link.setUrlDownload(newLink);
        }
    }
                ...
                if (filename == null) {
                    ...
                    if (filename == null) {
                        ...
                        if (filename == null) {
                            ...
                            if (filename == null) {
                                String fileID = new Regex(link.getDownloadURL(), "mangobit\\.com/([a-z0-9]{12})").getMatch(0);
                                if (fileID != null) {
                                    filename = new Regex(BRBEFORE, fileID + "\\.html\\]\\[IMG\\]http://mangobit\\.com/i/\\d+/(.*?)\\[/IMG\\]").getMatch(0);
                                    String emergencyPictureLink = new Regex(BRBEFORE, fileID + "\\.html\\]\\[IMG\\](http://mangobit\\.com/i/\\d+/.*?)\\[/IMG\\]").getMatch(0);
                                    if (emergencyPictureLink != null) link.setProperty(EMERGENCYPICTUREDOWNLOADLINKTEXT, emergencyPictureLink);
                                }
                            }
                        }
                    }
    ...
    public void doFree(DownloadLink downloadLink, boolean resumable, int maxchunks, boolean loggedIn) throws Exception, PluginException {
        boolean isPictureLink = br.containsHTML("Here To View Image\"");
        String dllink = null;
        ...
        if (error) {
            ...
            doSomething();
            dllink = getDllink();
            // If we have a server error but the picturelink was found before
            // and we can be sure that this link is a picture we might still
            // have a chance to download it here
            if (dllink == null && br.containsHTML(FATALSERVERERROR) && isPictureLink) {
                logger.warning("Emergency handling active, trying to download the picture...");
                dllink = downloadLink.getStringProperty(EMERGENCYPICTUREDOWNLOADLINKTEXT);
            }
            if (dllink == null) {
    ...
    public void checkErrors(DownloadLink theLink, boolean checkAll, String passCode, boolean loggedIn) throws NumberFormatException, PluginException {
        ...
        if (BRBEFORE.contains(FATALSERVERERROR)) throw new PluginException(LinkStatus.ERROR_TEMPORARILY_UNAVAILABLE, "Server error!", 10 * 60 * 1000l);
